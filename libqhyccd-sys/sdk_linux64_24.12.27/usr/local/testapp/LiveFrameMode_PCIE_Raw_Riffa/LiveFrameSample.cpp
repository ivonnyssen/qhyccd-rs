...
/*
 ...
 if(GetQHYCCDPcieDDRNum((qhyccd_handle *)camhandle)>16777216)   {
   
      QHYCCDPcieRecv(camhandle, buffer,15*1024*1024,25000);
      
      //printf("buffer %x %x %x %x %x %x %x %x\n",buffer[0],buffer[1],buffer[2],buffer[3],buffer[4],buffer[5],buffer[6],buffer[7]);
      if ((buffer[0] == 0x44) && (buffer[1] == 0x44) && (buffer[2] == 0x55) && (buffer[3] == 0x55))
      {
